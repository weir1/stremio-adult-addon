...
class Scraper1337x {
    ...
    generateTorrentId(name, link) {
        ...
        return `x_${Buffer.from(combined).toString('base64').replace(/[^a-zA-Z0-9]/g, '').substring(0, 20)}`;
    }
    ...
    async scrapeCategory(categoryPath) {
        for (const baseUrl of this.baseUrls) {
            try {
                ...
                enrichedTorrents = await this.enrichTorrentsWithTorbox(enrichedTorrents);
